...
export function ReviewSession() {
  ...
  const handleAnswer = useCallback(async (isCorrect: boolean) => {
    if (!currentExercise || isAnimating) return;
    setIsAnimating(true);
    ...
    // Only update DB if NOT in cramming mode
    if (!isCramming) {
  ...
  }, [currentExercise, certainty, examMode, incrementCorrect, incrementIncorrect, setAlertMessage, isAnimating, isCramming]);
  ...
  const endSession = useCallback(async () => {
    // Save session stats only if NOT cramming
    if (!isCramming && sessionStartTime) {
  ...
  }, [sessionStartTime, sessionCorrect, sessionIncorrect, currentSubject, currentCourse, currentPack, resetSession, setCurrentView, isCramming]);
    ...
    <div className="flex flex-col h-full">
      ...
      {isCramming && (
        <motion.div
          ...
          className="bg-gradient-to-r from-orange-500/20 to-yellow-500/20 border-b border-orange-500/30 px-4 py-2 flex items-center justify-center gap-2"
        >
          <Zap className="w-4 h-4 text-orange-400" />
          <span className="text-sm font-medium text-orange-300">
            Mode Cramming — Pas d&apos;impact sur tes stats
          </span>
        </motion.div>
      )}
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <motion.button
              ...
              className="p-2 rounded-lg hover:bg-zinc-800 text-zinc-500 hover:text-zinc-300 hidden md:flex"
      ...
      <AnimatePresence>
        {showShortcuts && (
          <motion.div
            ...
            animate={{ opacity: 1, y: 0 }}
            exit={{ opacity: 0, y: -10 }}
            className="absolute right-4 top-20 z-40 bg-zinc-900 border border-zinc-700 rounded-xl p-4 shadow-xl"
          >
            <h4 className="text-sm font-medium text-zinc-200 mb-3">⌨️ Raccourcis</h4>
            <div className="space-y-2 text-xs text-zinc-400">
              <div className="flex justify-between gap-6">
                <span>Révéler la réponse</span>
                <kbd className="px-2 py-0.5 bg-zinc-800 rounded text-zinc-300">Espace</kbd>
              </div>
              <div className="flex justify-between gap-6">
                <span>Juste</span>
                <kbd className="px-2 py-0.5 bg-zinc-800 rounded text-zinc-300">→ / K</kbd>
              </div>
              <div className="flex justify-between gap-6">
                <span>Faux</span>
                <kbd className="px-2 py-0.5 bg-zinc-800 rounded text-zinc-300">← / J</kbd>
              </div>
              <div className="flex justify-between gap-6">
                <span>Options (QCM)</span>
                <kbd className="px-2 py-0.5 bg-zinc-800 rounded text-zinc-300">1-4</kbd>
              </div>
              <div className="flex justify-between gap-6">
                <span>Quitter</span>
                <kbd className="px-2 py-0.5 bg-zinc-800 rounded text-zinc-300">Échap</kbd>
              </div>
            </div>
          </motion.div>
        )}
      ...
      <div className="flex-1 flex flex-col items-center justify-center p-6 overflow-auto">
        ...
        <motion.div
          ...
          exit={{ opacity: 0, x: -50 }}
          className={`w-full max-w-2xl ${isAnimating ? 'pointer-events-none opacity-80' : ''}`}
        >
          {renderExercise()}
        </motion.div>
        ...
        >
          <div className="text-center">
            ...
            <h2 className="text-2xl font-bold text-zinc-100 mb-2">
              {isCramming ? 'Cramming terminé !' : 'Session terminée !'}
            </h2>
            ...
            {isCramming && (
              <p className="text-orange-400 text-sm mb-4">
                🎯 Mode cramming — Tes stats n&apos;ont pas été modifiées
              </p>
            )}
            <motion.button
