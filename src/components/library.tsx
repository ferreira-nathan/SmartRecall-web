...
export function Library() {
  ...
  const handleStartCramming = useCallback(async (
    subject: Subject | null,
    course: Course | null,
    pack: Pack | null,
  ) => {
    setIsCramming(true);
    await handleStartReview(subject, course, pack, false);
          ...
          {viewLevel !== 'subjects' && (
            <div className="flex gap-2">
              ...
              <motion.button
                whileHover={{ scale: 1.02 }}
                whileTap={{ scale: 0.98 }}
                onClick={() => handleStartCramming(
                  subjects.find(s => s.id === selectedSubjectId) || null,
                  courses.find(c => c.id === selectedCourseId) || null,
                  null,
                )}
                className="px-4 py-2 bg-gradient-to-r from-orange-500/20 to-yellow-500/20 border border-orange-500/30 rounded-xl text-orange-300 flex items-center gap-2 hover:from-orange-500/30 hover:to-yellow-500/30 transition-all"
              >
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-2 mt-4">
                      <motion.button
                        ...
                        onClick={(e) => {
                          e.stopPropagation();
                          handleStartCramming(null, null, pack);
                        }}
                        className="py-2 px-3 bg-gradient-to-r from-orange-500/15 to-yellow-500/15 border border-orange-500/30 hover:from-orange-500/25 hover:to-yellow-500/25 rounded-xl text-sm text-orange-300 flex items-center justify-center gap-1.5 transition-all"
                        title="Mode Cramming"
                      >
