...
export async function getDueExercises(packId?: number, courseId?: number, subjectId?: number): Promise<Exercise[]> {
  ...
  if (courseId) {
    ...
    return db.exercises
      .where('nextReview')
      .belowOrEqual(now)
      .filter(ex => packIds.has(ex.packId))
      .toArray();
  }
  ...
  if (subjectId) {
    ...
    const packs = await db.packs.filter(p => courseIds.has(p.courseId)).toArray();
    const packIds = new Set(packs.map(p => p.id!));
    return db.exercises
      .where('nextReview')
      .belowOrEqual(now)
...
export async function getSettings(): Promise<Settings> {
  ...
  return { id: 1, examMode: '1month', soundEnabled: true, notificationsEnabled: false, reminderTime: '19:00', currentStreak: 0 };
}
